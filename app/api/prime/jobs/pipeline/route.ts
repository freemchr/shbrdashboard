...
export async function GET(req: NextRequest) {
  try {
    ...
    for (let i = 11; i >= 0; i--) {
      ...
      weeks.push({ week: key, label, start: key, end: weekEnd.toISOString().split('T')[0], count: 0 });
    }
    ...
    const counts = await pMap(weeks, async (w) => {
      try {
        ...
        const data = await primeGet(`/jobs?per_page=1&q=${qs}`) as { meta?: { pagination?: { total?: number } } };
    ...
    // Only cache if we got real data (at least one non-zero week)
    if (result.some(w => w.count > 0)) {
