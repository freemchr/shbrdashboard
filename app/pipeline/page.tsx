    ...
    <div>
      ...
      <div className="flex flex-wrap gap-3 mb-4">
        <input
          ...
          className="bg-gray-900 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-red-500 w-64"
        />
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-800 bg-gray-900">
                {['Job #', 'Address', 'Client Ref', 'Status', 'Type', 'Region', 'Auth. Total', 'Updated', ''].map(h => (
                  <th key={h} className={`py-3 px-3 text-xs text-gray-400 font-medium whitespace-nowrap ${h === 'Auth. Total' ? 'text-right' : 'text-left'}`}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {filtered.length === 0 ? (
                ...
                <tr key={job.id} className="border-b border-gray-800/50 hover:bg-gray-800/40 transition-colors">
                  ...
                  <td className="py-2.5 px-3 text-gray-300 max-w-[180px] truncate">{job.address}</td>
                  <td className="py-2.5 px-3 text-gray-400 text-xs whitespace-nowrap">{job.clientReference || '—'}</td>
                  ...
                  <td className="py-2.5 px-3 text-gray-400 text-xs whitespace-nowrap">{job.jobType}</td>
                  <td className="py-2.5 px-3 text-gray-400 text-xs whitespace-nowrap">{job.region}</td>
                  <td className="py-2.5 px-3 text-right text-gray-300 text-xs whitespace-nowrap">{formatCurrency(job.authorisedTotal)}</td>
